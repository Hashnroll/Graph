...
public:
	...
	Graph() {
		V = 0;
		E = 0;
		//adj.push_back(make_pair(0,deque<int>())); //0 вершина - не используется
	}


	void addVertex() { //добавить вершину
		adj.push_back(make_pair(V, deque<int>())); //вставка нового дека в вектор
		indegree.insert(pair<int, int>(V, 0));//изначально в вершину V не входят никакие вершины
	...
	void deleteVertex(int i) {
		...
		for (auto it = adj.begin(); it != adj.end(); it++) { //проход по вектору(по вершинам)
			deque<int>::iterator it_deque; //итератор для дека
			for (it_deque = it->second.begin(); it_deque != it->second.end(); it_deque++); { //проход по деку - по всем смежным вершинам для данной
				if (*it_deque == i) {
